...
export default function Home() {
  ...
  const handleAddProduct = () => {
    if (newProduct.productName && newProduct.quantity && newProduct.price) {
      const updatedStock = [...stockData, { ...newProduct, id: stockData.length + 1 }];
  ...
  return (
    <>
      <Header />
      <div className='container bg-red-50 mx-auto p-8'>
      <div className='mb-8'>
          ...
          <tbody>
            {stockData.map((item) => (
              <tr key={item.id}>
                ...
                <td className='border border-gray-500 px-4 py-2'>{item.price.toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
